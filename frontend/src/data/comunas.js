// Regiones y comunas de Chile — fuente: comunas.json
const regionesData = [
...
/**
 * Returns list of region names sorted alphabetically.
 */
export function getRegiones() {
  return regionesData.map((r) => r.region).sort((a, b) => a.localeCompare(b, 'es'));
}

/**
 * Returns comunas for a given region name, sorted alphabetically.
 * If region is empty/null returns an empty array.
 */
export function getComunasByRegion(regionName) {
  if (!regionName) return [];
  const found = regionesData.find((r) => r.region === regionName);
  return found ? [...found.comunas].sort((a, b) => a.localeCompare(b, 'es')) : [];
}
