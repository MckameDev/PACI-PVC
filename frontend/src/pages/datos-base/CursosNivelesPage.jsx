...
export default function CursosNivelesPage() {
  ...
  const fetchItems = useCallback(async (p = 1) => {
    setLoading(true);
    try {
      const res = await api.get('/cursos-niveles', { params: { page: p, limit: 20 } });
      ...
      setAlert({ type: 'error', message: 'Error al cargar cursos/niveles' });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openEdit = (item) => {
    ...
    setFormData({
      nombre: item.nombre || '',
      valor_numerico: item.valor_numerico?.toString() || '',
    });
    setFormErrors({});
    setFormModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (!formData.valor_numerico) errors.valor_numerico = 'El valor numérico es requerido';
    else if (isNaN(Number(formData.valor_numerico))) errors.valor_numerico = 'Debe ser un número';
    ...
    try {
      const payload = {
        ...
        valor_numerico: parseInt(formData.valor_numerico, 10),
      };
      if (editId) {
        await api.put(`/cursos-niveles/${editId}`, payload);
        setAlert({ type: 'success', message: 'Curso/Nivel actualizado' });
      } else {
        await api.post('/cursos-niveles', payload);
        setAlert({ type: 'success', message: 'Curso/Nivel creado' });
      }
      setFormModal(false);
      fetchItems(page);
    } catch (err) {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await api.patch(`/cursos-niveles/${deleteModal.id}`);
      setAlert({ type: 'success', message: 'Curso/Nivel eliminado' });
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <EmptyState
          icon={GraduationCap}
          title="Sin cursos/niveles"
          description={search ? 'No se encontraron resultados.' : 'Crea tu primer curso o nivel.'}
        >
          {!search && (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                ...
                <th className="px-4 py-3 text-center font-semibold text-slate-700">Valor Numérico</th>
      ...
      <Modal
        ...
        title={editId ? 'Editar Curso/Nivel' : 'Nuevo Curso/Nivel'}
        size="sm"
      >
        <div className="space-y-4">
          ...
          <Input
            ...
            error={formErrors.valor_numerico}
            placeholder="Ej: 1"
          />
          <p className="text-xs text-slate-500">
            El valor numérico define el orden del curso y se usa para calcular la diferencia (DIF) en la trayectoria de OA.
          </p>
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button size="sm" loading={saving} onClick={handleSave}>
              {editId ? 'Guardar Cambios' : 'Crear Curso/Nivel'}
            </Button>
          </div>
        </div>
      </Modal>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-slate-600">
            ¿Está seguro de eliminar el curso/nivel <strong>{deleteModal.nombre}</strong>?
          </p>
