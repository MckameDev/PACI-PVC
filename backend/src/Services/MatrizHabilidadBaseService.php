...
use App\Models\MatrizHabilidadBase;
use App\Helpers\Validator;
use App\Helpers\Response;

class MatrizHabilidadBaseService
{
    private MatrizHabilidadBase $model;

    public function __construct()
    {
        $this->model = new MatrizHabilidadBase();
    }
    ...
    public function create(array $data, ?string $userId): array
    {
        ...
        if (!$this->model->isUnique('nombre', $data['nombre'])) {
            Response::error('La habilidad base ya existe', 409);
        }
